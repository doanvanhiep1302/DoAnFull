...
public class ProductAdminController {
    ...
    @PatchMapping("update-product-history")
    public ResponseEntity<ApiResponse> updateProductHistory(@RequestBody ProductHistory productHistory) {
        ApiResponse response;
        try {
            productService.updateProductHistory(productHistory);
            response = new ApiResponse(ErrorCode.SUCCESS);
        }
        catch (CustomException e) {
            response  = new ApiResponse(e);
